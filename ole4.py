...
def calculate(mylist):
    ...
    while x<len(mylist)-4 and y<len(mylist)-1 :
        ...
        if mylist[x+4] == mylist[x]+4:
            ...
            x = x + 1
    return bottleneck, avg, blist[0],blist[1],blist[2]
...
dfe = dfd.withColumn("BottleneckCount",col('vals.val1')).withColumn("AverageDuration",col('vals.val2')).withColumn("Bottleneck1 start",col('vals.val3')).withColumn("Bottleneck2 start",col('vals.val4')).drop("lowSpeed","vals")
...
dffinal2.coalesce(1).write.csv("/reactor/davami/olefeed7/", header=True)
